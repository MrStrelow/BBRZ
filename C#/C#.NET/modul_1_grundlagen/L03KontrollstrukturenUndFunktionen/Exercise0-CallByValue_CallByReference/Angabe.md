...
### int und ref int als Parameter einer Methode
...
### Teilaufgabe 1 - *TryParse* und ``Variablen`` des ``Typs`` *int*
...
### Teilaufgabe 2 - *TryParse* und ``Variablen`` des ``Typs`` *Enum*
...
## Array und Seiteneffekte
**Kurze Wiederholung**: ``Arrays`` sind ``Referenztypen``. Wenn ein ``Array`` an eine ``Methode`` übergeben wird, wird die ``Referenz`` auf das ``Array`` übergeben. Was das genau bedeutet sehen wir in späteren Modulen. Änderungen an den Elementen des ``Arrays`` innerhalb der ``Methode`` sind daher auch außerhalb sichtbar. Dies nennt man einen ``Seiteneffekt``. Auch schon in den vorherigen Übungen ist mit dem ``Keyword`` ``out`` ein ``Seiteneffekt`` verwendet worden.
