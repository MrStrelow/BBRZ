...
public class CustomerService
{
    ...
    private readonly IHttpContextAccessor _httpContextAccessor; // Neu

    public CustomerService(ApplicationDbContext context, ILogger<CustomerService> logger, IHttpContextAccessor httpContextAccessor)
    {
        _context = context;
        _logger = logger;
    ...
    public async Task CreateOrderAsync(OrderDto orderDto)
    {
        ...
        // --- NEU: Exception für Business Regel "Hohe Beträge nur für Admins" ---
        if (totalAmount > 50.00m)
        {
            var user = _httpContextAccessor.HttpContext?.User;

            // Wenn User nicht eingeloggt ist ODER nicht in Rolle Admin
            if (user == null || !user.IsInRole("Admin"))
            {
                _logger.LogWarning("Bestellung über 50€ von Nicht-Admin abgelehnt.");
                throw new FruehstueckBusinessException($"Bestellungen über 50€ erfordern Admin-Rechte. (Aktuell: {totalAmount:C})");
            }
        }
