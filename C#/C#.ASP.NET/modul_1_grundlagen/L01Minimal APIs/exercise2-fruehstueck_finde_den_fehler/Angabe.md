# Angabe: Frühstücksrestaurant Webanwendung - ein Fehler ist passiert... aber wo?

Betrachte die gleiche Angabe wie aus [Exercise 1](../exercise1-fruehstueck_mit_minimal_api/Angabe.md), jedoch haben wir hier etwas verändert. Finde den Fehler:
* mit dem *Inspect* tools eures Browsers und
* mithilfe des ``Debugers``.

## Aufgabe 1 - Inspect Tool des Browsers
...
## Aufgabe 2 -  
Verwene diese Information um mit den ``Debugger`` genauer in die Methoden zu schauen. Stoppe den Server mit dem roten viereck in VS und starten diesen neu mit dem **ausgefüllten dreieck**. Setze einen ``Breakpoint`` an einem sinnvollen Ort (was sagt dir Aufgabe 1?) und schau dir die Variablen an. Wo ist der Fehler und wie beheben wir diesen?

## Vorlage Program.cs

Verwende diesmal folgende, **geänderte** Vorlage. Falls die Lösung benötigt wird, suche nach der Zeile ``// HIER IST DER FEHLER! Aber wo genau?``.
...
//      
//      // Daten aus der In-Memory-dbContext holen
//      ...
...
app.MapPost(
    ...
    (
        ...
        [FromForm] List<int> selectedMenuIds,
        [FromForm] List<int> selectedDishIds
    ) =>
    {
