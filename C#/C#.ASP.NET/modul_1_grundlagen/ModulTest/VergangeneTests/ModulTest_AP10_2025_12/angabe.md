...
3) Wo werden bei einem ``Request`` der ``http-Methode`` *get* die ``Parameter`` übermittelt?
4) Was bedeutet der ``Serverstatus`` *200*, *300*, *400* und *500*? Wo kann ich diese finden?
...
* ``DishController``: Dieser beinhaltet eine ``Methode`` *Details* und eine ``Methode`` *Create*. 
    * *Details*: 
        1) Diese nimmt die ``http-Methode`` *get* für einen spezielles ``Model`` *Dish* entgegen. Diese ``Action`` gibt ein ``ViewModel`` *DishViewModel* mit der ``Eigenschaft`` *Id*, *Name*, *Price*, *Preperationstep* und *Ingredients* des ``Model`` *Dish* von der ``View`` an den ``Controller`` weiter. Entsprechende Fehlermeldungen sollen mit den ``Attributen`` an die ``View`` mithilfe des *ModelStates* weitergegeben werden. Der Aufruf der ``Action`` *Details* wird durch einen Klick auf ein *Dish* in der List aller *Dishes* auf der "Hauptseite aufgerufen. **Das Verhalten der Hauptseite (Links einfügen mit ``<a asp-controller="..." asp-action="...">ein link</a>``) muss angepasst werden.**

* ``MenuController``: Dieser beinhaltet eine ``Methode`` *Create* ohne ``Parameter`` und ``Attribut`` *HttpGet* und eine ``Methode`` *Create* mit ``Parameter`` *MenuViewModel viewModel* und ``Attribut`` *HttpPost*. 
    * *Create für Get*: 
        1) Durch einen klick auf einen Link auf der Hauptseite wird die ``View`` der ``Action`` *Create* mit einem ``http-Request`` *Get* aufgerufen.
    * *Create für Post*: 
        2) Diese nimmt die ``http-Methode`` *post* entgegen. Als ``Parameter`` wird ein ``ViewModel`` *MenuViewModel* empfangen. Wandle dieses mithilfe von der ``Methode`` *ToDto* zu einem ``DTO`` *MenuDto* um. Verwende den ``Service`` *MenuService* mit der ``Methode`` *CreateMenu* welches ein ``DTO`` *MenuDto* als ``Parameter`` entgegenimmt. Leite dann mit einem *Redirect()* an die ``Action`` *Index* des ``Controllers`` *FruehstueckController* weiter. 
    
* ``DishViewModel``: Die im *DishController* implementierte ``Action`` *Details* bekommt das *DishViewModel* als ``Parameter`` übermittelt. Innerhlab des ``ViewModel`` *DishViewModel* soll ein ``Attribut`` verwendet werden um die Gültigkeit der ``Eigenschaft`` *Id* sicherzustellen. Prüfe hier auf *Range(1, int.MaxValue, ...)* und verwende *Required(...)*. Implementiere zudem eine ``Methode`` *ToDto* welche aus einem ``ViewModel`` *DishViewModel* ein ``DTO`` *DishDto* macht.

* ``MenuViewModel``: Die im *MenuController* implementierte ``Action`` *Create* mit ``Attribut`` *HttpPost* bekommt das *MenuViewModel* als ``Parameter`` übermittelt. Innerhlab des ``ViewModel`` *MenuViewModel* sind die ``Eigenschaften`` *Name*, *AvailableDishes* und *SelectedDishIds* anzulegen. Prüfe hier bei *Name* auf *StringLength(100, MinimumLength = 2, ErrorMessage ...)* und verwende *Required(...)*. Implementiere zudem eine ``Methode`` *ToDto* welche aus einem ``ViewModel`` *MenuViewModel* ein ``DTO`` *MenuDto* macht.
...
* ``MenuDto``: Dieses soll die ``Eigenschaften`` *Name* und *DishIds* besitzen. Es wird für die Kommunikation mit dem ``Service`` *MenuService* verwendet.

* ``MenuService``: Der ``Parameter`` der ``Methode`` *CreateMenu* ist ein ``DTO`` *MenuDto*. Erstelle ein neues ``Model`` *Menu* und belege die ``Eigenschaften`` *Name* und *Preis*. Übernimm aus dem ``DTO`` *MenuDto* die ``Eigenschaft`` *Name* und verwende ``LINQ`` um die ``Eigenschaft`` *Preis* als Summe der Preise der ausgewählten Dishes.

* ``View``: Verwende aus der **Vorlage** das ``cshtml`` mit Namen *Details* welches im Ordner *Dish* liegt, sowie das ``cshtml`` mit Namen *Create* welches im Ordner *Menu* liegt. Dieser Code ist auskommentiert, kommentiere diesen ein um fortzufahren. Es muss nur mit ``Razor-Syntax`` *@* adaptiert werden. Es muss **kein** neuer html-code erstellt werden. Implementiere zudem im ``cshtml`` mit Namen *Index* welches im Ordner *Fruehstueck* liegt,den aufruf der ``http-Requests`` *get* für *Menu* und *Dish*. 

Das zu implmentierende Verhalten in Bildern: 
...
3) Was erlaubt uns die ``Razor``-``Syntax`` *@* in einem ``cshtml``-file? Ein Benutzer geht ins Kontextmenü (Rechtsklick) und navigiert auf der Websitezu zu *Quellcode anzeigen* (CTRL+u). Finden wir in dem angezeigten ``html-file`` die ``Razor``-``Syntax`` *@* wie z.B. *@model*, *@for* und *@if*? 