...
public class Kunde
{
    ...
    public void ValidateKundeGuardClause()
    {
        ...
        // Wir gehen den "Pfad des Programmes" nach.
        // ... Hier finden wir wenn Produkt.IstPremium wahr ist, unerwünschte Zustände.
        if (Produkt.IstPremium && Bestellhistorie <= 20)
            throw new InvalidOperationException("❗ Kunde hat zu wenig Bestellungen für Premium-Produkte.");

        if (Produkt.IstPremium && Produkt.Preis > Budget)
            throw new InvalidOperationException("❗ Kunde darf das Premium-Produkt nicht kaufen. Zu wenig Geld.");

        // ... Hier finden wir wenn Produkt.IstPremium falsch ist, unerwünschte Zustände.
        if (!Produkt.IstPremium && Produkt.Preis > Budget && !IstKreditwürdig)
            throw new InvalidOperationException("❗ Kunde benötigt einen Kredit, da das Produkt zu teuer ist.");
