...
- **Anwendungsfall**: Geeignet für schnellen Indexzugriff und wenn die Anzahl der Elemente variieren kann.
- **Vor- und Nachteile**:
  - **Vorteile**: Schneller Zugriff über Index (`O(1)`).
  - **Nachteile**: Einfügen/Löschen in der Mitte ist teuer (`O(n)`).

### LinkedList<T>
- **Implementierung**: Doppelt verkettete Liste.
- **Anwendungsfall**: Bei häufigem Einfügen/Löschen am Anfang oder in der Mitte.
- **Vor- und Nachteile**:
  - **Vorteile**: Effizientes Einfügen/Löschen an beliebigen Stellen (`O(1)` bei bekannter Position).
...
- **Vor- und Nachteile**:
  - **Vorteile**: Schneller Zugriff (`O(1)` im Durchschnitt).
