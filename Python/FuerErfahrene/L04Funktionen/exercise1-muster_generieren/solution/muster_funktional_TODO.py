...
def fill_canvas(field, symbol):
    return list(map(lambda row: [symbol for _ in range(len(row))], field))
...
def copy(field):
    return list(map(lambda row: row[:], field))

def rotate(field):
    return transpose(mirror_x(field))

def transpose(field):
    return list(map(lambda *args: [args[i] for i in range(len(args))], *field))

def mirror_x(field):
    return list(reversed(field))

def mirror_y(field):
    return list(map(lambda row: row[::-1], field))
